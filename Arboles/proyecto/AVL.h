...
int alturaAVL(AVL arbol)
{
    if(arbol==NULL) return 0;
         ...
         int altIzq=0;
         int altDer=0;
         if(arbol->izq) altIzq=alturaAVL(arbol->izq);
         if(arbol->der) altDer=alturaAVL(arbol->der);
...
void equilibrar(AVL &r, AVL &nodoAVL)
{
   ...
   {
    nodoAVL->FE=alturaAVL(nodoAVL->der)-alturaAVL(nodoAVL->izq);
   //COMPROBAR FACTOR DQUILIBRIO DE LOS nodoAVLS
   if(nodoAVL->FE == 0) salir = TRUE;
   else
       if(nodoAVL->FE == -2) {
                   if(nodoAVL->izq->FE == 1){ verArbolAVL(r,0); RDD(r, nodoAVL);  verArbolAVL(r,0);}
                   else{ verArbolAVL(r,0);RSD(r, nodoAVL);printf("%s\n", "\n...Rotacion simple a la derecha realizada..."); verArbolAVL(r,0);}
                   salir = TRUE;
                   }
       else if(nodoAVL->FE == 2){
                   if(nodoAVL->der->FE == -1){verArbolAVL(r,0); RDI(r, nodoAVL);printf("%s\n", "\n...Rotacion doble a la izquierda realizada..."); verArbolAVL(r,0);}
                   else {verArbolAVL(r,0); RSI(r, nodoAVL);printf("%s\n", "\n...Rotacion simple a la izquierda realizada..."); verArbolAVL(r,0);}
                   salir = TRUE;
                   }
